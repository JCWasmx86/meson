...
Attempts to format your code. It is possible some comments aren't readded.
...
- `kwa_ml`: Make every dictionary literal and function call with kwargs expand into multiple lines. **Default: false**
- `wide_colon`: Whether to put a space before the colon operator (ex. `key : val`) **Default: false**
- `no_single_comma_function`: Don't add a trailing comma to multi-line function calls with only one argument. **Default: false**
